package com.anwen.mongo.conditions.interfaces.aggregate.pipeline.group;
...
public class Accumulators {
     ...
     * @since mongodb.driver.manual reference/operator/aggregation/sum/ $sum
     */
    public static <TExpression,T,R> BsonField sum(final SFunction<T,R> fieldName, final TExpression expression) {
        return accumulatorOperator("$sum", fieldName.getFieldNameLine(), expression);
    }
    ...
    /**
     * Gets a field name for a $group operation representing the result of merging {@link BsonField}s of the documents.
     * If documents to merge include the same field name, {@link BsonField}, in the resulting document, has the value from the last document
     * merged for {@link BsonField}.
     *
     * @param fieldName {@link BsonField} name
     * @param expression the expression
     * @param <TExpression> the expression type
